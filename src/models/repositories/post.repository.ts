...
export class PostRepository extends Repository<Post> {
  async getPostByStatusAndKeyword(keyword: string, status: PostStatus, page: number, limit: number) {
  ...
  async getPostByKeywordAndType(keyword: string, topic: Topics, page: number, limit: number) {
    const queryBuilder = this.createQueryBuilder('post')
      .leftJoinAndSelect('post.user', 'user')
      .leftJoinAndSelect('post.tourGuide', 'tourGuide')
    if (keyword) {
