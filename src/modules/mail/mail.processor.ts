...
export class MailProcessor {
  ...
  async sendRegisterMail({ data }: Job<RegisterEmailDto>): Promise<number> {
    this.logger.log(`Start job: sendUpdateEmail user ${data.username} email ${data.email}`)
    const context = {
      email: data.email,
      token: data.token,
    ...
    this.logger.log(`Done job: sendUpdateEmail ${data.email} email ${data.username}`)
    return 1
  }

  @Process('sendCreateAdminMail')
  async sendCreateAdminMail({ data }: Job<CreateAdminDto>): Promise<number> {
    this.logger.log(`Start job: sendCreateAdminMail user ${data.username} email ${data.email}`)
    const context = {
      email: data.email,
      url: data.url,
      username: data.username,
  ...
  async sendAcceptOrderMail({ data }: Job<ApproveOrderEmailDto>): Promise<number> {
    this.logger.log(`Start job: sendAcceptOrderMail user ${data.username} email ${data.email}`)
    const context = {
      email: data.email,
    ...
    this.logger.log(`Done job: sendAcceptOrderMail ${data.email} email ${data.username}`)
    return 1
  }
  @Process('sendRejectOrderMail')
  async sendRejectOrderMail({ data }: Job<ApproveOrderEmailDto>): Promise<number> {
    this.logger.log(`Start job: sendRejectOrderMail user ${data.username} email ${data.email}`)
    const context = {
      email: data.email,
      tourGuideName: data.tourGuideName,
    }
    ...
    this.logger.log(`Done job: sendRejectOrderMail ${data.email} email ${data.username}`)
    return 1
  }

  @Process('sendCreatMeetingMail')
  async sendCreatMeetingMail({ data }: Job<CreateMeetingDto>): Promise<number> {
    this.logger.log(`Start job: sendCreatMeetingMail user ${data.name} email ${data.email}`)
    const context = {
      email: data.email,
      name: data.name,
      date: data.date,
    ...
    this.logger.log(`Done job: sendCreatMeetingMail ${data.email} email ${data.username}`)
    return 1
  }
