...
import { AdminGetPostDto, GetPostDto } from './dto/get-post.dto'
import { UpdateBlogDto } from './dto/update-post.dto'
import { AdminApproveRequest, UpdateStatusBlogDto } from './dto/update-status-post.dto'
@Injectable()
export class PostService {
  ...
  async getPostByStatus(options: AdminGetPostDto): Promise<Response> {
    ...
    const tourGuides = await this.postRepository.getPostByStatusAndKeyword(keyword, status, page, limit)
    return {
  ...
  async getListPost(options: GetPostDto): Promise<Response> {
    ...
    const posts = await this.postRepository.getPostByKeywordAndType(keyword, topics, page, limit)
    return {
      ...httpResponse.GET_POST_SUCCESS,
      returnValue: BasePaginationResponseDto.convertToPaginationWithTotalPages(
        posts,
  ...
  async adminUpdateStatusPost(body: UpdateStatusBlogDto) {
    ...
    if (![PostStatus.ACTIVE, PostStatus.REJECTED].includes(status)) {
      throw new HttpException(httpErrors.INVALID_PARAMS, HttpStatus.BAD_REQUEST)
    }
    const post = await this.postRepository.findOne({
      where: {
        id: postId,
  ...
  async createPost(body: CreateBlogDto, actorId: number, role: ActorRole): Promise<Response> {
    ...
    if (!actor) {
      throw new HttpException(
        role === ActorRole.USER ? httpErrors.USER_NOT_FOUND : httpErrors.TOUR_GUIDE_NOT_FOUND,
  ...
  async updatePost(body: UpdateBlogDto, actorId: number, role: ActorRole): Promise<Response> {
  ...
  async approveUpdateRequest(body: AdminApproveRequest): Promise<Response> {
    ...
    await this.postRepository.update(
      { id: post.id },
      {
        ...
        currentContent: action === AdminAction.APPROVE ? post.updateContent : post.currentContent,
        updateContent: null,
      },
    )
  ...
  async deletePost(postId: number, actorId: number, role: ActorRole) {
    ...
    if (role === ActorRole.ADMIN) {
      ...
      const postIdActor = role === ActorRole.USER ? post.user.id : post.tourGuide.id
