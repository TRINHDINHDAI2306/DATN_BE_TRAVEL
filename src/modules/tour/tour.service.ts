...
import { AdminApproveAction, TourStatus, TourTypes } from 'src/shares/enum/tour.enum'
...
export class TourService {
  ...
  async createTour(body: CreateTourDto, tourGuideId: number): Promise<Response> {
  ...
  async getTours(options: GetTourDto): Promise<Response> {
    ...
    const data = await this.tourRepository.findAndCount({
      ...
      relations: ['images', 'rates', 'tourGuide', 'userFavorites', 'tourSchedule', 'province'],
      take: options.limit,
  ...
  async approveTour(body: ApproveTourDto): Promise<Response> {
    ...
    if (tour.tourGuide.verifyStatus !== TourguideStatus.ACTIVE) {
      throw new HttpException(httpErrors.TOUR_GUIDE_NOT_ACTIVE, HttpStatus.BAD_REQUEST)
    }
