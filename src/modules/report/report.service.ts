...
import { GetReportStatus, HandleReportPostAction, ReportStatus } from 'src/shares/enum/report.enum'
...
export class ReportService {
  ...
  async getReportPost(options: GetReportDto): Promise<Response> {
    ...
    return {
      ...
      returnValue: BasePaginationResponseDto.convertToPaginationWithTotalPages(reports, page, limit),
    }
  }
  ...
  async deleteReport(reportId: number): Promise<Response> {
    ...
    if (!report) {
      throw new HttpException(httpErrors.REPORT_NOT_FOUND, HttpStatus.NOT_FOUND)
    }
  ...
  async handleReportPost(body: HandleReportPostDto): Promise<Response> {
    ...
    if (action === HandleReportPostAction.SKIP) {
      await this.reportRepository.update({ id: reportId }, { status: ReportStatus.PROCESSED })
    } else {
      const reportIds = report.post.reports.map((e) => e.id)
      await Promise.all([
        this.reportRepository.update({ id: In([...reportIds]) }, { status: ReportStatus.PROCESSED }),
  ...
  async setupMeeting(body: CreateMeetingReportDto): Promise<Response> {
    ...
    const report = await this.reportRepository.findOne({
      ...
      relations: ['post', 'reportedBy', 'order', 'order.tourGuide', 'order.tour'],
    })
    if (!report) {
      throw new HttpException(httpErrors.REPORT_NOT_FOUND, HttpStatus.NOT_FOUND)
    }
    await this.reportRepository.update({ id: reportId }, { status: ReportStatus.PROCESSING, meetingDate })
    this.mailService.sendCreatMeetingMail({
      ...
      date: new Date(meetingDate).toISOString().slice(0, 10).split('-').reverse().join('-'),
  ...
  async skipReport(reportId: number): Promise<Response> {
    ...
    if (report.status === ReportStatus.PROCESSING && today < new Date(report.meetingDate)) {
  ...
  async handleBanReportTourguide(reportId: number): Promise<Response> {
    ...
    // hoafn tienfg
    await Promise.all([
      this.tourGuideRepository.update({ id: report.order.tourGuide.id }, { verifyStatus: TourguideStatus.REJECT }),
      this.userRepository.update(
        { id: report.order.user.id },
        {
          balance: report.order.user.balance + report.order.price * (system.returnUserPercent / 100),
          availableBalance: report.order.user.availableBalance + report.order.price * (system.returnUserPercent / 100),
        },
      ),
      ...
      this.reportRepository.update({ id: report.id }, { status: ReportStatus.PROCESSED }),
    ])
    return httpResponse.HANLED_REPORT
  }
