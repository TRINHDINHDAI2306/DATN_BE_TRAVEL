...
export class CronTask {
  ...
  private async changePendingTransaction() {
    ...
    const filtered = transactions.filter((transaction) => {
      ...
      const isTimeDifferenceGreaterThan15Minutes = timeDifference.asMinutes() > 15
      return isTimeDifferenceGreaterThan15Minutes
    })
    // Output: false
    await Promise.all(
      filtered.map((e) => {
        this.transactionRepository.update({ id: e.id }, { status: TransactionStatus.FAILED })
      }),
    )
  }
  private async changeOrderNotPrePaid() {
    ...
    for (const order of orderNotPrepaid) {
      task.push(this.orderRepository.update({ id: order.id }, { status: OrderStatus.REJECTED }))
      task.push(
      ...
      task.push(
        this.tourGuideRepository.update(
          ...
          {
            availableBalance: () => `tour_guides.available_balance + ${order.tourGuideDeposit}`,
