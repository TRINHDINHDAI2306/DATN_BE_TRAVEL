...
import { AdminChangeStatusUserDto } from './dto/change-user-status.dto'
import { AdminGetUsersDto } from './dto/get-list-user.dto'
...
export class UserService {
  ...
  async getUserPost(options: BasePaginationRequestDto, userId: number): Promise<Response> {
  ...
  async changeUserStatus(body: AdminChangeStatusUserDto): Promise<Response> {
    ...
    await this.userRepository.update(
      { id: userId },
      {
        verifyStatus: status === ChangeStatus.ACTIVE ? UserStatus.ACTIVE : UserStatus.LOCKED,
      },
    )
  ...
  async getUserTransaction(options: GetTransactionDto, userId: number): Promise<Response> {
  ...
  async IPNUrl(query) {
    const { vnp_Amount, vnp_ResponseCode, vnp_TransactionStatus, vnp_TxnRef } = query
