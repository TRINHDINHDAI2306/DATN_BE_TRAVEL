...
export class UserController {
  ...
  async getUserPost(@Query() options: BasePaginationRequestDto, @ActorID() actorId: number): Promise<Response> {
    return this.userService.getUserPost(options, actorId)
  }

  @Put('/')
  @UseGuards(AdminModAuthGuard)
  async changeStatusUser(@Body() body: AdminChangeStatusUserDto): Promise<Response> {
  ...
  @Get('/transaction')
  @UseGuards(UserAuthGuard)
  async getTransaction(@Query() options: GetTransactionDto, @ActorID() userId: number): Promise<Response> {
