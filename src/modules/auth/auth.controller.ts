...
export class AuthController {
  ...
  async registerTourguide(@Body() body: RegisterTourguideDto): Promise<Response> {
    return this.authService.registerTourguide(body)
  }

  @Post('/resend-confirmation-email')
  async resendConfirmationEmail(body: ResendEmailRegisterDto): Promise<Response> {
    return this.authService.resendRegisterEmail(body)
  }

  @Put('/active-user/:token')
  async activeUser(@Param('token') token: string, @Body() body: ActiveUserDto): Promise<Response> {
  ...
  async getMe(@ActorID() userId: number, @ActorRoleDecorator() actorRole: ActorRole): Promise<Response> {
