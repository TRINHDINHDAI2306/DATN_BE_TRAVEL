import { Body, CACHE_MANAGER, HttpException, HttpStatus, Inject, Injectable } from '@nestjs/common'
...
import { MailService } from '../mail/mail.sevice'
...
import { IJwtAdminPayload, IJwtPayload, IJwtTourguidePayload } from './interfaces/payload.interface'
...
export class AuthService {
  ...
  async register(body: RegisterDto): Promise<Response> {
    ...
    if (user && [UserStatus.ACTIVE, UserStatus.LOCKED].includes(user.verifyStatus)) {
    ...
    if (token) {
      throw new HttpException(httpErrors.WAIT_TO_RESEND, HttpStatus.BAD_REQUEST)
    }
  ...
  async registerTourguide(body: RegisterTourguideDto): Promise<Response> {
    const { name, username, email, password, phone, dob, gender, provinces } = body
  ...
  async activeAccount(token: string, email: string): Promise<Response> {
    ...
    if (!checkToken) {
      throw new HttpException(httpErrors.REGISTER_TOKEN_NOT_FOUND, HttpStatus.NOT_FOUND)
    }
    if (!user) {
      throw new HttpException(httpErrors.USER_NOT_FOUND, HttpStatus.NOT_FOUND)
    }
    if (token != checkToken) {
      throw new HttpException(httpErrors.REGISTER_TOKEN_NOT_MATCH, HttpStatus.BAD_REQUEST)
    }
    await Promise.all([
      this.userRepository.update({ email: email }, { verifyStatus: UserStatus.ACTIVE }),
  ...
  async userLogin(body: LoginDto) {
    ...
    if (!userExisted) throw new HttpException(httpErrors.USER_LOGIN_FAIL, HttpStatus.BAD_REQUEST)
    ...
    const comparePassword = await bcrypt.compare(password, userExisted.password)

    if (!comparePassword) throw new HttpException(httpErrors.USER_LOGIN_FAIL, HttpStatus.BAD_REQUEST)
  ...
  async tourGuideLogin(@Body() body: LoginDto): Promise<Response> {
    ...
    if (!tourguideExisted) throw new HttpException(httpErrors.USER_LOGIN_FAIL, HttpStatus.BAD_REQUEST)

    const comparePassword = await bcrypt.compare(password, tourguideExisted.password)
  ...
  async adminLogin(body: LoginDto): Promise<Response> {
    ...
    if (!adminExisted) throw new HttpException(httpErrors.USER_LOGIN_FAIL, HttpStatus.BAD_REQUEST)

    if (adminExisted.status === AdminStatus.INACTIVE)
      throw new HttpException(httpErrors.USER_NOT_ACTIVE, HttpStatus.BAD_REQUEST)

    const comparePassword = await bcrypt.compare(password, adminExisted.password)
    ...
    const payload = {
      ...
      role: adminExisted.role === AdminRole.ADMIN ? ActorRole.ADMIN : ActorRole.MOD,
  ...
  async forgotPassword(body: ForgotPasswordDto) {
    ...
    if (!checkOTP || otp != checkOTP) {
      throw new HttpException(httpErrors.FORGOT_PASSWORD_OTP_NOT_MATCH, HttpStatus.NOT_FOUND)
    }
    if (!user) {
      throw new HttpException(httpErrors.USER_NOT_FOUND, HttpStatus.NOT_FOUND)
    }
    ...
    switch (actorRole) {
      case ActorRole.USER:
        me = await this.userRepository.findOne({
          ...
          relations: ['userVouchers', 'userFavorites', 'orders', 'transactions', 'userVouchers.voucher'],
        })
        break
