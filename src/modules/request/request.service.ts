...
import { GetUserRequestDto } from './dto/get-request.dto'
...
export class RequestService {
  ...
  async createRequest(body: CreateRequestDto, userId: number): Promise<Response> {
  ...
  async getMyRequest(options: GetUserRequestDto, userId: number): Promise<Response> {
  ...
  async tourGuideGetRequest(options: BasePaginationRequestDto, tourGuideId: number): Promise<Response> {
    ...
    const tourGuideProvinces = tourGuide.provinces.map((province) => province.id)
    const requests = await this.userRequestRepository.findAndCount({
      where: {
  ...
  async getOneRequest(requestId: number, actorId: number, role: ActorRole): Promise<Response> {
