...
export class AdminService {
  ...
  async getAdminOrModByIdAndUsername(id: number, username: string): Promise<Admin> {
    const admin = await this.adminRepository.findOne({
      where: {
        id,
        username,
  ...
  async activeAccount(body: ActiveAdminDto): Promise<Response> {
    ...
    await Promise.all([
      this.adminRepository.update({ email: email }, { status: AdminStatus.ACTIVE, password: passwordHash }),
