...
import { CommentDto, GetCommentDto, UpdateCommentDto } from './dto/comment.dto'
...
export class CommentService {
  ...
  async createComment(body: CommentDto, actorId: number, actorRole: ActorRole): Promise<Response> {
  ...
  async getPostComment(options: GetCommentDto): Promise<Response> {
    ...
    const comment = await this.commentRepository.findAndCount({
      where: { post },
      relations: ['user', 'post', 'parrentComment', 'parrentComment.subcomments'],
      take: options.limit,
    ...
    return {
      ...
      returnValue: BasePaginationResponseDto.convertToPaginationWithTotalPages(comment, page, limit),
    }
  }

  async updateComment(body: UpdateCommentDto, actorId: number, actorRole: ActorRole) {
    ...
    if (actorRole === ActorRole.USER) {
      ...
      if (!comment) {
        throw new HttpException(httpErrors.COMMENT_NOT_FOUND, HttpStatus.NOT_FOUND)
      }
  ...
  async deleteComment(commentId: number, actorId: number, actorRole: ActorRole): Promise<Response> {
    if (actorRole === ActorRole.USER) {
      const user = await this.userRepository.findOne({
        id: actorId,
        verifyStatus: UserStatus.ACTIVE,
      ...
      if (comment.subcomments && comment.subcomments.length > 0) {
        const subcommentIds = comment.subcomments.map((subcomment) => subcomment.id)
