...
import { GetTransactionDto, GetTransactionWithdrawDto } from 'src/shares/dtos/get-transaction.dto'
...
import { AproveActionWithdraw, TransactionStatus, TransactionType } from 'src/shares/enum/transaction.enum'
...
import { AdminAproveWithdrawRequest } from './dto/admin-prove.dto'
...
export class TransactionService {
  ...
  async userWithdraw(body: WithdrawDto, userId: number): Promise<Response> {
    ...
    await Promise.all([
      this.userRepository.update({ id: userId }, { availableBalance: +user.availableBalance - +amount }),
  ...
  async tourGuideWithdraw(body: WithdrawDto, tourGuideId: number): Promise<Response> {
    const { amount } = body
    ...
    await Promise.all([
      this.tourGuideRepository.update({ id: tourGuideId }, { availableBalance: +tourGuide.availableBalance - +amount }),
      this.transactionRepository.insert({
        tourGuide: tourGuide,
        status: TransactionStatus.WAITING,
        amount,
  ...
  async getListWithdrawRequest(options: GetTransactionWithdrawDto): Promise<Response> {
    const { limit, page, startDate, endDate, isHistory } = options
    const transactions = await this.transactionRepository.findAndCount({
      where: {
        time: Between(new Date(startDate), new Date(moment(endDate).add(1, 'day').toString())),
        status: isHistory ? Not(TransactionStatus.WAITING) : TransactionStatus.WAITING,
        type: TransactionType.WITHDRAW,
      },
    ...
    return {
      ...
      returnValue: BasePaginationResponseDto.convertToPaginationWithTotalPages(transactions, page, limit),
    }
  }
  ...
  async getTransactionsByDateRange(dto: GetTransactionDto): Promise<Response> {
    ...
    const query = this.transactionRepository
      ...
      .where('transaction.updatedAt >= :startDate AND transaction.updatedAt <= :endDate', {
    ...
    const [goodRates, badRates, profit, orders] = await Promise.all([
      this.rateRepository
        ...
        .where('rate.star >= 4')
        .andWhere('rate.createdAt >= :startDate AND  rate.createdAt <= :endDate', {
          startDate: startDateDb,
          endDate: endDateDb,
        })
        .getCount(),
      ...
      this.orderRepository
        ...
        .andWhere('order.updatedAt >= :startDate AND order.updatedAt <= :endDate', {
          startDate: startDateDb,
          endDate: endDateDb,
        })
        .getCount(),
    ...
    return {
      ...httpResponse.GET_TRANSACTION_SUCCESS,
      returnValue: BasePaginationResponseDto.convertToPaginationWithTotalPages([data, countData], page, limit),
      options: {
        goodRates,
        badRates,
  ...
  async getMyListWithdraw(actorId: number, role: ActorRole, options: GetTransactionWithdrawDto): Promise<Response> {
    const { limit, page, startDate, endDate, isHistory } = options
    switch (role) {
      case ActorRole.TOURGUIDE: {
        ...
        const tourGuideTransactions = await this.transactionRepository.findAndCount({
          where: {
            time: Between(new Date(startDate), new Date(moment(endDate).add(1, 'day').toString())),
            status: isHistory ? Not(TransactionStatus.WAITING) : TransactionStatus.WAITING,
            tourGuide,
        ...
        return {
          ...httpResponse.GET_TRANSACTION_SUCCESS,
          returnValue: BasePaginationResponseDto.convertToPaginationWithTotalPages(tourGuideTransactions, page, limit),
        }
      }
      case ActorRole.USER: {
        const user = await this.userRepository.findOne({
  ...
  async approveRequestWithdraw(body: AdminAproveWithdrawRequest, actorId: number): Promise<Response> {
    ...
    if (!withdraw) {
      throw new HttpException(httpErrors.REQUEST_NOT_FOUND, HttpStatus.NOT_FOUND)
    }
    switch (action) {
      case AproveActionWithdraw.ACCEPT:
        if (withdraw.tourGuide !== null) {
          ...
          await Promise.all([
            this.userRepository.update({ id: withdraw.user.id }, { balance: withdraw.user.balance - withdraw.amount }),
            // this.systemRepository.update(
            //   { id: system.id },
            //   {
            //     balance: system.balance - withdraw.amount,
        ...
        if (withdraw.tourGuide !== null) {
          await Promise.all([
            this.tourGuideRepository.update(
              { id: withdraw.tourGuide.id },
              {
                availableBalance: withdraw.tourGuide.availableBalance - withdraw.amount,
              },
            ),
            this.transactionRepository.update(
              { id: withdrawId },
          ...
          await Promise.all([
            this.userRepository.update(
              { id: withdraw.user.id },
              {
                availableBalance: withdraw.user.availableBalance - withdraw.amount,
              },
            ),
            this.transactionRepository.update(
              { id: withdrawId },
