...
export class TransactionController {
  ...
  async userWithdraw(@Body() body: WithdrawDto, @ActorID() userId: number): Promise<Response> {
    return this.transactionService.userWithdraw(body, userId)
  }

  @Post('/tourguide-withdraw')
  @UseGuards(TourGuideAuthGuard)
  async tourGuideWithdraw(@Body() body: WithdrawDto, @ActorID() tourGuideId: number): Promise<Response> {
    return this.transactionService.tourGuideWithdraw(body, tourGuideId)
  }

  @Get('/request-withdraw')
  @UseGuards(AdminModAuthGuard)
  async getRequestList(@Query() options: GetTransactionWithdrawDto): Promise<Response> {
    return this.transactionService.getListWithdrawRequest(options)
  }

  @Put('/request-withdraw')
  @UseGuards(AdminModAuthGuard)
  async approveRequest(@Body() body: AdminAproveWithdrawRequest, @ActorID() actorId: number): Promise<Response> {
  ...
  async getMyWithdraw(
    ...
    return this.transactionService.getMyListWithdraw(actorId, actorRole, options)
  }

  @Get('/admin/')
  @UseGuards(AdminModAuthGuard)
  async adminGetTransctions(@Query() options: GetTransactionDto): Promise<Response> {
